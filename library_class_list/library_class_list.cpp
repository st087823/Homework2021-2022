...
int main()
{
    ...
    {
        if (choose == 1) //по номеру добавить абонента в конец имеющегося списка
        {
            Subscriber a;
            cin >> a;
            l.push_back(a);
        ...
        if (choose == 2) // удал абонент
        {
            int tmp = 0;
            cin >> tmp; // номер удаляемого абонент tmp

            list <Subscriber> ::iterator i; //интератор на начало списка ставим
            i = l.begin();
            int n = 0;
            for (auto x : l) // ищем нужный номер tmp
            {
                n++;
            ...
            /*Cначала переменная x принимает значение первого элемента списка и выполняется тело цикла,
            затем − второго, и снова выполняется тело цикла, и так далее, до последнего элемента списка.*/

            advance(i, n - 1); // сдвигает итератор i на n-1 позиций вперед? тк. изначально i = l.begin();
        ...
        if (choose == 3) // распечатать имя и адрес абонента по номеру
        {
            ...
            for (auto x : l)
            {
                if (x.num == tmp)
                {
                    cout << x; //выводим то на что указывает итератор
                }
            }
        }
        menu();
